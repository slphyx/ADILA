...
#####################################
# scripts undated on 22 October 2024
...
model_A <- function(params){
  with(
    as.list(params),
    {
      ...
      names(pen.ddd)       <- paste("Penicillins")
      names(beta.enz.ddd)  <- paste("Beta-lactam antibiotics plus enzyme inhibitor")
      names(cep.first.ddd) <- paste("First generation cephalosporins")
      names(amino.ddd)     <- paste("Aminoglycosides")
      names(imida.ddd)     <- paste("Nitroimidazoles")
      names(tetra.ddd)     <- paste("Tetracyclines")
      names(amphe.ddd)     <- paste("Amphenicols")
      names(linco.ddd)     <- paste("Lincosamides")
      
      names(cep.second.third.ddd) <- paste("Second/ Third generation cephalosporins")
      names(macro.ddd)            <- paste("Macrolides")
      names(beta.enz.pseudo.ddd)  <- paste("Beta-lactam antibiotics plus enzyme inhibitor: Anti-pseudomonal")
      names(quino.ddd)            <- paste("Fluroquinolones")
      names(carba.ddd)            <- paste("Carbapenems")
      names(glyco.ddd)            <- paste("Glycopeptides")
