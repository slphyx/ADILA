...
app_server <- function(session,input, output) {
  ...
  input_big <- reactive({
    ...
    if(input$choices_ac == "child"){
      ...
      p.no.resp.cap.para <- binomi.para(p.no.resp.cap.est, std_err)
      ...
      p.hiv.para <- binomi.para(p.hiv.est, 0.01)
  ...
  output$summary_inputs2 <- renderTable({
    req(!is.null(input_big()$para_data))
    input_big()$para_data
  })
