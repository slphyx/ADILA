...
app_ui <- fluidPage(
  titlePanel("Multi-Drug Antibiotic Choice Model with Tabs"),
  
  sidebarLayout(
    sidebarPanel(
      numericInput("num_drugs", "Number of Drugs", value = 1, min = 1, max = 5, step = 1),
      actionButton("submit", "Submit")  # Button to calculate results
    ),
    
    mainPanel(
      uiOutput("drug_tabs"),   # Tabs for input fields
